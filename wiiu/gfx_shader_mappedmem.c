...
GX2PixelShader* WHBGfxLoadGFDPixelShaderMappedMem(uint32_t index, const void *file)
{
   ...
   {
      WHBLogPrintf("%s: index %u >= %u GFDGetPixelShaderCount(file)", __FUNCTION__, index, GFDGetPixelShaderCount(file));
      goto error;
   }
   ...
   if (!shader)
   {
      WHBLogPrintf("%s: MEMAllocFromMappedMemoryEx(%u, 64) failed", __FUNCTION__, headerSize);
      goto error;
   }
...
GX2VertexShader* WHBGfxLoadGFDVertexShaderMappedMem(uint32_t index, const void *file)
{
   ...
   {
      WHBLogPrintf("%s: index %u >= %u GFDGetVertexShaderCount(file)", __FUNCTION__, index, GFDGetVertexShaderCount(file));
      goto error;
   }
